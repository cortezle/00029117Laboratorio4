void createLocalA(Matrix &A,mesh m){
    float t = m.getParameter(CONS_T)/4;
...
void createLocalD(Matrix &D,mesh m){
    float uve = m.getParameter(CONS_UVE);
    float l = m.getParameter(ELEMENT_LENGTH);

    D.at(0).at(0) += uve/l; D.at(0).at(1) += -uve/l;
    D.at(1).at(0) += -uve/l; D.at(1).at(1) += uve/l;
}
...
Vector createLocalb(int element,mesh &m){
    ...
    float psi = m.getParameter(PSI), l = m.getParameter(ELEMENT_LENGTH), n = m.getParameter(CONS_N);
